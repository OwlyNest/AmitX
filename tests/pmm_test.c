...
static void test_alloc_free(void) {
  ...
  PFN_T free_before = pmm_get_free_frames();

  PHYS_ADDR_T p = pmm_alloc_frame();
  if (!p) {
...
static void test_contiguous(void) {
  ...
  PHYS_ADDR_T p = pmm_alloc_frames(4);
  if (!p) {
...
static void test_null_free(void) {
  ...
  pmm_free_frame((PHYS_ADDR_T)NULL);
...
static void test_reserve_unreserve(void) {
  ...
  if (pmm_get_free_frames() != (PFN_T)free_before) {
...
void test_pmm_aligned(void) {
  ...
  PHYS_ADDR_T p = pmm_alloc_aligned(1, 4);
  if (!p) {
