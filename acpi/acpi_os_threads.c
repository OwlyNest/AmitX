...
ACPI_STATUS AcpiOsExecute(ACPI_EXECUTE_TYPE Type, ACPI_OSD_EXEC_CALLBACK Function, void *Context) {
    (void)Type;
    task_t *task = task_create_arg((void (*)(void *))Function, Context, "acpi-exec");
    return task ? AE_OK : AE_NO_MEMORY;
}

void AcpiOsSleep(UINT64 Milliseconds) {
    task_sleep((uint32_t)Milliseconds);
}
...
void AcpiOsStall(UINT32 Microseconds) {
    ...
    for (i = 0; i < loops; i++) asm volatile ("nop");
}
...
ACPI_STATUS AcpiOsAcquireMutex(ACPI_MUTEX Handle, UINT16 Timeout) {
    ...
    if (Timeout == 0) return mutex_trylock(m) ? AE_OK : AE_TIME;
...
ACPI_STATUS AcpiOsDeleteSemaphore(ACPI_SEMAPHORE Handle) {
    free((semaphore_t *)Handle);
    return AE_OK;
}

ACPI_STATUS AcpiOsWaitSemaphore(ACPI_SEMAPHORE Handle, UINT32 Units, UINT16 Timeout) {
    semaphore_t *s = (semaphore_t *)Handle;
    if (Timeout == 0) return sem_trywait(s, Units) ? AE_OK : AE_TIME;
    uint32_t ms = (Timeout == 0xFFFF) ? SEM_WAIT_FOREVER : Timeout;
    return sem_wait(s, Units, ms) ? AE_OK : AE_TIME;
}

ACPI_STATUS AcpiOsSignalSemaphore(ACPI_SEMAPHORE Handle, UINT32 Units) {
    sem_signal((semaphore_t *)Handle, Units);
    return AE_OK;
}
...
ACPI_CPU_FLAGS AcpiOsAcquireLock(ACPI_SPINLOCK Handle) {
    return (ACPI_CPU_FLAGS)spinlock_acquire((spinlock_t *)Handle);
}

void AcpiOsReleaseLock(ACPI_SPINLOCK Handle, ACPI_CPU_FLAGS Flags) {
    spinlock_release((spinlock_t *)Handle, (uint32_t)Flags);
}
