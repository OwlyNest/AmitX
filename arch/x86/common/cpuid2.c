/*
 * arch/x86/common/cpuid2.c - [Enter description]
 * Author:   amity
...
static const ULONG simple_basic[] = {0x00000001, 0x00000002, 0x00000003,
 ...
 *                                                                          *
 ========================================================================== */
static const cpuid_raw_t *cpuid_raw_find(const cpuid_raw_db_t *db, ULONG leaf,
                                         ULONG subleaf) {
...
static inline void cpuid_exec(cpuid_raw_db_t *db, ULONG leaf, ULONG subleaf) {
  ...
  db->entries[db->count++] = (cpuid_raw_t){.leaf = leaf,
                                           .subleaf = in_subleaf,
                                           .eax = ra,
                                           .ebx = rb,
                                           .ecx = rc,
...
void cpuid_dump_db(void) {
  ...
  for (ULONG idx = 0; idx < db.count; idx++) {
    ...
    printk("%#010x %-3u %#010x %#010x %#010x %#010x\n", leaf->leaf,
           leaf->subleaf, leaf->eax, leaf->ebx, leaf->ecx, leaf->edx);
  }
}
