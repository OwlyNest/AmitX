/*
	* fs/smkfs/file.c - File I/O and File Descriptor Management
	* Author:   amity
...
int smkfs_read(uint64_t record_id, uint64_t offset, size_t len, void *buf) {
    ...
    if (!mounted || !buf || record_id == 0) return SMKFS_ERR_INVAL;
...
int smkfs_open(const char *path, int flags) {
    ...
    if (path_lookup(path, &record_id) != 0) {
        if (flags & SMKFS_O_CREATE) {
            if (smkfs_create_file(path, SMKFS_PERM_WRITE | SMKFS_PERM_WRITE) != 0) {
                return SMKFS_ERR_NOSPC;
			}
...
int smkfs_close(int fd) {
    ...
    if (!fd_table[fd].used) return SMKFS_ERR_INVAL;
...
int smkfs_seek(int fd, int64_t offset, int whence) {
    ...
    switch (whence) {
		...
		case SMKFS_SEEK_END:
			if (record_read(fd_table[fd].record_id, &rec, attr_buf,
					sizeof(attr_buf)) == 0) {
				if (record_find_attr(attr_buf, SMKFS_ATTRT_FSIZE,
						(void **)&fsize_ptr, NULL) == 0) {
					file_size = *fsize_ptr;
				}
			}
...
int smkfs_create_file(const char *path, uint16_t permissions) {
    ...
    if (!last_slash || last_slash == path + 2) {
        ...
        parent_path[len] = '\0';
        if (path_lookup(parent_path, &parent) != 0) {
            return SMKFS_ERR_NOTFOUND;
		}
