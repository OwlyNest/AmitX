/*
    * arch/x86/scheduler.c - Priority-based round-robin scheduler
    * Author:   amity
...
void scheduler_add(task_t *task) {
    ...
    if (prio >= TASK_NUM_PRIOS) prio = TASK_NUM_PRIOS - 1;
...
void timer_wheel_remove(task_t *task) {
    if (!task) return;
    if (task->timer_bucket >= TIMER_WHEEL_SIZE) return;
...
void scheduler_reap(void) {
    for (int i = 0; i < TASK_MAX_TASKS; i++) {
        ...
        if (t && t->state == TASK_ZOMBIE) {
            ...
            if (t == task_current()) continue;
            if (t == ready_queue[TASK_PRIO_IDLE]) continue;
