...
ACPI_STATUS AcpiOsReadMemory(ACPI_PHYSICAL_ADDRESS Address, UINT64 *Value,
  ...
  switch (Width) {
  case 8:
    *Value = *(volatile uint8_t *)v;
    break;
  case 16:
    *Value = *(volatile uint16_t *)v;
    break;
  case 32:
    *Value = *(volatile uint32_t *)v;
    break;
  case 64:
    *Value = *(volatile uint64_t *)v;
    break;
  default:
...
ACPI_STATUS AcpiOsWriteMemory(ACPI_PHYSICAL_ADDRESS Address, UINT64 Value,
  ...
  switch (Width) {
  case 8:
    *(volatile uint8_t *)v = Value;
    break;
  case 16:
    *(volatile uint16_t *)v = Value;
    break;
  case 32:
    *(volatile uint32_t *)v = Value;
    break;
  case 64:
    *(volatile uint64_t *)v = Value;
    break;
  default:
    vmm_unmap_physical(v, width_bytes);
    return AE_BAD_PARAMETER;
