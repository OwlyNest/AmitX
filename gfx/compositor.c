...
void compositor_blit_window(window_t *win) {
    ...
    for (uint32_t row = 0; row < win->surface.height; row++) {
        ...
        uint32_t *src = win->surface.pixels +
                        row * win->surface.pitch_px;
        uint32_t *dst = fb.back.pixels +
                        fb_y * fb.back.pitch_px + x0;
