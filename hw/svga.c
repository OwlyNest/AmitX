...
static int svga_find_device(void) {
  pci_device_t *dev =
      pci_get_device(SVGA_PCI_VENDOR_VMWARE, SVGA_PCI_DEVICE_SVGA2);
  if (!dev) {
  ...
  uint32_t cmd =
      pci_read_config(dev->bus, dev->device, dev->function, PCI_COMMAND);
  cmd |= 0x07;
...
static int svga_map_fb(void) {
  ...
  printk("[svga] FB  0x%08x+0x%05x -> 0x%08x (%u KB)\n", svga.fb_phys,
         fb_offset, SVGA_FB_VIRT, fb_size / 1024);
  return 0;
}
...
int svga_set_resolution(uint32_t width, uint32_t height, uint32_t bpp) {
  ...
  printk("[svga] Actual mode: %ux%u @ %ubpp pitch=%u\n[svga] Masks: R=%08x "
         "G=%08x B=%08x\n",
         svga.width, svga.height, svga.bpp, svga.pitch, svga.red_mask,
         svga.green_mask, svga.blue_mask);
...
int svga_init(void) {
  ...
  if (svga_set_resolution(target_w, target_h, 32) != 0)
    return -1;
  if (svga_map_fb() != 0)
    return -1;
  if (svga_fifo_init() != 0)
  ...
  printk("[svga] Mode set: %ux%u pitch=%u bpp=%u VRAM %u KB\n", svga.width,
         svga.height, svga.pitch, svga.bpp, svga.vram_size / 1024);
  printk("[svga] Oracle debug: pitch=%u width=%u bpp=%u\n"
         "[svga] pitch / width = %u (expected 3 for 24bpp, 4 for 32bpp)\n"
         "[svga] pitch %% width = %u\n",
         svga.pitch, svga.width, svga.bpp, svga.pitch / svga.width,
         svga.pitch % svga.width);

  if (fb_init() != 0) {
    printk("[svga] Failed to switch to the SVGA framebuffer\n");
    return -1;
  }
  return 0;
}
