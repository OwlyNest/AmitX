/*
	* include/fs/smkfs_internal.h - SmKFS Internal Shared Declarations
	* Author:   amity
...
/* --- Globals ---*/
extern smkfs_superblock_t sb;
extern int mounted;
extern uint8_t drive_num;
extern uint64_t journal_next_sequence;
extern int journal_in_transaction;
extern uint64_t journal_write_pos;
extern smkfs_fd_t fd_table[SMKFS_FD_MAX];
/* --- Prototypes ---*/
/* --- Block I/O --- */
...
void bitmap_clear(uint64_t block);
int bitmap_test(uint64_t block);
uint64_t bitmap_alloc_range(uint32_t count);
uint64_t bitmap_alloc(void);
void bitmap_free_range(uint64_t start, uint32_t count);

/* --- Record --- */
int record_read(uint64_t record_id, smkfs_record_t *rec, void *attr_buf, size_t buf_size);
int record_write(uint64_t record_id, const smkfs_record_t *rec, const void *attr_buf);
uint64_t record_alloc(uint16_t object_type);
void record_free(uint64_t record_id);
