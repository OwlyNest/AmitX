...
/* Multiboot2 ACPI tags: bootloader hands us a raw copy of whatever
 * RSDP it found — EBDA/BIOS-ROM scan on legacy boot, EFI
 * configuration-table walk on UEFI. We don't need to know which. */
typedef struct mb2_tag_acpi {
  mb2_tag_t tag;
  uint8_t rsdp[0];
...
typedef struct mb2_tag_framebuffer {
  ...
  uint32_t height;
  uint8_t bpp;
  uint8_t type; /* 0=indexed, 1=RGB direct, 2=text */
  uint16_t reserved;
...
#define MB2_TAG_FOREACH(mb, tag)                                               \
  for ((tag) = (mb2_tag_t *)((uint8_t *)(mb) + 8); (tag)->type != MB2_TAG_END; \
...
#define MULTIBOOT_MMAP_FOR_EACH(mb, entry)                                     \
  for ((entry) = (multiboot_mmap_entry_t *)(uintptr_t)(mb)->mmap_addr;         \
       ...
       (entry) =                                                               \
           (multiboot_mmap_entry_t *)((uint32_t)(entry) + (entry)->size + 4))
