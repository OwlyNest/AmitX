...
SMKFS_STATUS smkfs_link(_SMKFS_MOUNT *mnt, SMKFS_PATH existing_path,
  ...
  if (record_read(mnt, existing_id, &rec, attr_buf, sizeof(attr_buf)) !=
      SMKFS_OK) {
    journal_abort(mnt);
    return SMKFS_ERR_IO;
  }
  ...
  if (record_read(mnt, new_parent, &parent_rec, parent_attr,
  ...
  record_add_attr(parent_attr, sizeof(parent_attr), SMKFS_ATTRT_DATA, &new_root,
                  sizeof(new_root));
