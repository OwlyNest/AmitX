...
void update_hardware_cursor() { set_cursor(cursor_col, cursor_row); }
...
void setcolor(uint8_t fg, uint8_t bg) { color = (bg << 4) | (fg & 0x0F); }
...
void draw_statusbar() {
  ...
  while (*name && pos < 79)
    status_text[pos++] = *name++;
  if (pos < 79)
    status_text[pos++] = ' ';
  while (*version && pos < 79)
    status_text[pos++] = *version++;
  if (pos < 79)
    status_text[pos++] = ' ';
  if (pos < 79)
    status_text[pos++] = '(';
  while (*build_date && pos < 79)
    status_text[pos++] = *build_date++;
  if (pos < 79)
    status_text[pos++] = ')';
  status_text[pos] = '\0';
...
void draw_box(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t fg,
              uint8_t bg) {
  if (width < 2 || height < 2)
    return;
...
void draw_list(uint8_t x, uint8_t y, uint8_t width, uint8_t height,
  ...
  for (uint8_t i = 0; i < visible; i++) {
    ...
    for (uint8_t j = 0; j < width - 2; j++) {
      ...
      if (c == '\0')
        break;
      video_memory[(y + 1 + i) * VGA_WIDTH + x + 1 + j] = (row_color << 8) | c;
    }
  }
}
