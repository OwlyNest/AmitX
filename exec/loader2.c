...
static int read_file(const char *path, uint8_t **out_buf, uint32_t *out_size) {
    ...
    uint32_t pass1_size = AMX2_HEADER_SIZE + header.section_count * AMX2_SECTION_SIZE + header.string_size;
    ...
    amx2_section_t *sects = (amx2_section_t *)malloc(
        header.section_count * sizeof(amx2_section_t));
    if (!sects) {
        free(pass1);
        return -1;
    }
    ...
    if (header.reloc_count > 0) {
        uint32_t end = header.reloc_offset
                     + header.reloc_count * AMX2_RELOC_SIZE;
        if (end > max_file_end) {
            max_file_end = end;
        }
    }
    ...
    if (header.import_count > 0) {
        uint32_t end = header.import_offset + header.import_count * AMX2_IMPORT_SIZE;
        if (end > max_file_end) {
            max_file_end = end;
        }
    }
    ...
    if (header.export_count > 0) {
        uint32_t end = header.export_offset + header.export_count * AMX2_EXPORT_SIZE;
        if (end > max_file_end) {
            max_file_end = end;
        }
    }
...
int exec2_load(const char *path, exec2_context_t *ctx) {
    ...
    if (ctx->header.section_count > 0) {
        ctx->sections = (amx2_section_t *)malloc(
            ctx->header.section_count * sizeof(amx2_section_t));
        if (!ctx->sections) {
            free(buf);
            return -1;
        }

        if (amx2_read_sections(buf, &ctx->header, ctx->sections, ctx->header.section_count) != 0) {
            free(ctx->sections);
            free(buf);
            return -1;
        }

        if (amx2_verify_sections(&ctx->header, ctx->sections)
            != AMX2_OK) {
            free(ctx->sections);
            free(buf);
            return -1;
...
int exec2_relocate(exec2_context_t *ctx) {
    ...
    if (ctx->header.reloc_offset
        + ctx->header.reloc_count * AMX2_RELOC_SIZE > ctx->file_size) {
        return -1;
    }

    amx2_reloc_t *table = (amx2_reloc_t *)(ctx->file_image + ctx->header.reloc_offset);
...
int exec2_resolve_imports(exec2_context_t *ctx) {
    ...
    if (ctx->header.import_offset
        + ctx->header.import_count * AMX2_IMPORT_SIZE > ctx->file_size) {
        return -1;
    }
    ...
    ctx->import_table = (void **)malloc(
        ctx->header.import_count * sizeof(void *));
    if (!ctx->import_table) {
        return -1;
    }

    amx2_import_t *imports = (amx2_import_t *)(ctx->file_image + ctx->header.import_offset);
...
int exec2_start(exec2_context_t *ctx) {
    ...
    void (*entry)(void) = (void (*)(void))(
        (uint32_t)(uintptr_t)ctx->image_base + entry_vaddr);
