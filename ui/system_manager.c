/*
	* ui/system_manager.c - System Manager (windowed)
	* Author:   amity
...
static void sm_draw_list(void) {
    ...
    /* Main panel fills most of window */
    int panel_x = 8;
    int panel_y = 8;
    int panel_w = SM_W - 16;
    int panel_h = SM_H - 44;
    ...
    gfx_title_bar(&win->surface, panel_x, panel_y, panel_w, " System Manager ");
    ...
    for (int i = 0; i < visible && (start + i) < (int)kscope_get_count(); i++) {
        ...
        gfx_fill_rect(&win->surface, list_x, row_y, list_w, SM_ROW_H, row_bg);
        ...
        window_draw_text(sm, list_x + 4, row_y + 6, line, row_fg);
    }

    gfx_status_bar(&win->surface, 0, SM_H - 28, SM_W, " [Up/Down] Select  [Enter] Details  [q] Back ");
}
...
static void sm_draw_detail(const kscope_node_t *node) {
    ...
    window_draw_text(sm, x, y, buf, fg); y += 16;
    ...
    window_draw_text(sm, x, y, buf, fg); y += 24;

    window_draw_text(sm, x, y, "Dependencies:", fg); y += 16;
    if (node->require_count != 0) {
        ...
        window_draw_text(sm, x, y, "    - none", fg); y += 16;
    }

    y += 8;
    window_draw_text(sm, x, y, "Capabilities:", fg); y += 16;
    if (node->provide_count != 0) {
        ...
        window_draw_text(sm, x, y,
            "    - Why in the name of Owly does this exist?", fg);
        y += 16;
    }

    gfx_status_bar(&win->surface, 0, SM_H - 28, SM_W, " [q] Back ");
}
...
void system_manager_run(void) {
    int win_x = (fb.back.width - SM_W) / 2;
    int win_y = (fb.back.height - SM_H) / 2;

    sm = window_create(win_x, win_y, SM_W, SM_H, "System Manager", WIN_FLAG_TITLEBAR);
    if ((int)sm == WIN_INVALID) {
        printk("[sm] Failed to create window\n");
        return;
    }

    win = window_get(sm);
    if (!win) {
        printk("[sm] Failed to get window\n");
        return;
    }
