...
static void z_order_push_top(window_handle_t handle) {
    if (z_count >= WIN_MAX_WINDOWS) return;
...
window_handle_t window_create(int x, int y, int w, int h, const char *title, uint32_t flags) {
    ...
    size_t buf_size = (size_t)win->surface.width * win->surface.height * sizeof(uint32_t);
...
void window_destroy(window_handle_t handle) {
    ...
    if (slot >= WIN_MAX_WINDOWS) return;
...
window_t *window_get(window_handle_t handle) {
    uint32_t slot = WIN_HANDLE_ID(handle);
    if (slot >= WIN_MAX_WINDOWS) return NULL;
...
void window_show(window_handle_t handle) {
    window_t *win = window_get(handle);
    if (!win) return;
    if (win->state == WIN_STATE_VISIBLE) return;
...
void window_focus(window_handle_t handle) {
    ...
    if (!target) return;
    if (target->state != WIN_STATE_VISIBLE) return;
...
window_handle_t window_at(int x, int y) {
    ...
    for (int i = z_count - 1; i >= 0; i--) {
        window_t *win = window_get(z_order[i]);
        if (!win) continue;
        if (win->state != WIN_STATE_VISIBLE) continue;
...
window_handle_t window_get_z_top(void) {
    if (z_count <= 0) return WIN_INVALID;
    return z_order[z_count - 1];
}

window_handle_t window_get_z_at(int index) {
    if (index < 0 || index >= z_count) return WIN_INVALID;
...
void window_draw_text(window_handle_t handle, uint32_t x, uint32_t y, const char *str, uint32_t color) {
    ...
    while (*str) {
        ...
        if (cx + 8 <= win->surface.width &&
            cy + 8 <= win->surface.height) {
