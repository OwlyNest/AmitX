...
VOID block_cache_init(_SMKFS_MOUNT *mnt) {
  ...
  if (!mnt->block_cache) {
    mnt->block_cache = (_SMKFS_BLOCK_BUF *)malloc(sizeof(_SMKFS_BLOCK_BUF) *
                                                  SMKFS_CACHE_SLOTS);
    if (!mnt->block_cache)
      return;
  }
...
SMKFS_STATUS block_cache_read(_SMKFS_MOUNT *mnt, SMKFS_BLOCK block,
  ...
  for (ULONG i = 0; i < SMKFS_CACHE_SLOTS; i++) {
    if (mnt->block_cache[i].valid && mnt->block_cache[i].block == block) {
...
SMKFS_STATUS block_cache_flush(_SMKFS_MOUNT *mnt) {
  ...
  for (i = 0; i < SMKFS_CACHE_SLOTS; i++) {
    if (mnt->block_cache[i].valid && mnt->block_cache[i].dirty) {
      if (disk_write_block(mnt, mnt->block_cache[i].block,
                           mnt->block_cache[i].data) != SMKFS_OK) {
        status = SMKFS_ERR_IO;
      } else {
        mnt->block_cache[i].dirty = 0;
      }
